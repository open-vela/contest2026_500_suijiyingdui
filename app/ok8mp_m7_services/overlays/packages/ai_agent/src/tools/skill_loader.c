...
#define BUILTIN_OK8MP_VOICE_LIGHT \
    "# OK8MP Voice Light Control\n\n" \
    "Control and inspect the RGB LED and buzzer on the OK8MP offline voice module.\n\n" \
    "## When to use\n" \
    "When the user asks to inspect the voice module, change its feedback color, " \
    "acknowledge a spoken command, or enable/disable its buzzer.\n\n" \
    "## How to use\n" \
    "1. Call voice_status before changing hardware\n" \
    "2. Call set_voice_rgb with red, green, blue values in 0..255\n" \
    "3. Call set_voice_buzzer with enabled=true or enabled=false when requested\n" \
    "4. Confirm returned hardware state; never claim success when ok=false\n\n" \
    "## Example\n" \
    "User: Check the voice module and set the light purple.\n" \
    "-> voice_status\n" \
    "-> set_voice_rgb {\"red\":128,\"green\":0,\"blue\":128}\n"

#define BUILTIN_OK8MP_NETWORK_GUARDIAN \
    "# OK8MP Network Guardian\n\n" \
    "Diagnose Ethernet-to-Internet HTTPS/TLS and show the result on the voice-module LED.\n\n" \
    "## When to use\n" \
    "When an online Agent request fails or real-board HTTPS/TLS must be verified.\n\n" \
    "## How to use\n" \
    "1. voice_status, then set_voice_rgb blue (0,0,255)\n" \
    "2. run_shell ping -c 3 192.168.2.1\n" \
    "3. run_shell nslookup example.com\n" \
    "4. run_shell https_client probe example.com 443\n" \
    "5. run_shell https_client get example.com /\n" \
    "6. Require certificate verification and HTTP 2xx; DNS/TCP alone is not success\n" \
    "7. Set green (0,255,0) on success; otherwise set red (255,0,0), enable buzzer, " \
    "and report the first failed layer\n"
...
static const builtin_skill_t s_builtins[] = {
    ...
    { "ok8mp-voice-light-control", BUILTIN_OK8MP_VOICE_LIGHT },
    { "ok8mp-network-guardian", BUILTIN_OK8MP_NETWORK_GUARDIAN },
#endif
};
