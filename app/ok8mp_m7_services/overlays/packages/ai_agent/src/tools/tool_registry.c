...
#endif
#ifdef CONFIG_AI_AGENT_OK8MP_NETWORK_TOOL
#include "tools/tool_ok8mp_network.h"
#endif
#ifdef CONFIG_AI_AGENT_OK8MP_CAN_TOOL
...
int tool_registry_init(void)
{
...
#ifdef CONFIG_OK8MP_M7_VOICE_I2C
    /* OK8MP-M7 custom voice module and uORB hardware tools. */
    REGISTER_TOOL_NO_PARAMS(
        "voice_status",
        "Read the OK8MP voice module version, busy state, word count and latest command ID.",
        tool_ok8mp_voice_status_execute);

    REGISTER_TOOL(
        "set_voice_rgb",
        "Set the RGB LED on the OK8MP voice module. Each channel is 0..255.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_NUM("red", "Red channel 0..255") ","
            TOOL_PARAM_NUM("green", "Green channel 0..255") ","
            TOOL_PARAM_NUM("blue", "Blue channel 0..255")
            TOOL_SCHEMA_END_REQUIRED("\"red\",\"green\",\"blue\""),
        tool_ok8mp_voice_rgb_execute);

    REGISTER_TOOL(
        "set_voice_buzzer",
        "Enable or disable the buzzer on the OK8MP voice module.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_BOOL("enabled", "True to enable the buzzer")
            TOOL_SCHEMA_END_REQUIRED("\"enabled\""),
        tool_ok8mp_voice_buzzer_execute);

    REGISTER_TOOL(
        "wait_voice_command",
        "Wait for one OK8MP voice command published on the openvela uORB topic.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_NUM("timeout_ms", "Timeout from 1 to 60000 ms")
            TOOL_SCHEMA_END(),
        tool_ok8mp_wait_voice_execute);
#endif

#ifdef CONFIG_AI_AGENT_OK8MP_NETWORK_TOOL
    REGISTER_TOOL_NO_PARAMS(
        "network_health_check",
        "Resolve example.com and perform a strict X.509-verified HTTPS GET. "
        "Returns structured DNS, TLS and HTTP evidence.",
        tool_ok8mp_network_health_execute);
#endif

#ifdef CONFIG_AI_AGENT_OK8MP_CAN_TOOL
    REGISTER_TOOL_NO_PARAMS(
        "can_health_check",
        "Read OK8MP FlexCAN1/can_service controller health, error counters and recovery evidence. It does not transmit a frame.",
        tool_ok8mp_can_health_execute);

    REGISTER_TOOL_NO_PARAMS(
        "can_read_status",
        "Read simulated vehicle state, coolant temperature, battery voltage and DTC counts through UDS DID F100 on OK8MP M7 FlexCAN1.",
        tool_ok8mp_can_read_status_execute);

    REGISTER_TOOL_NO_PARAMS(
        "can_read_dtc",
        "Read active and stored diagnostic trouble codes through UDS service 0x19.",
        tool_ok8mp_can_read_dtc_execute);

    REGISTER_TOOL_NO_PARAMS(
        "can_diagnostic",
        "MANDATORY LIVE CAN PROBE. For any request to diagnose ECU online status, CAN connectivity, or can_diagnostic, call this tool in the current turn; never answer from conversation history. It sends 0x700 A5 01 5A and validates live ECU response 0x708 5A 01 5A 01.",
        tool_ok8mp_can_diagnostic_execute);

    /* Compatibility names retained for prompts and demonstrations already
     * written against the first CAN Tool registration.
     */
    REGISTER_TOOL_NO_PARAMS(
        "can_vehicle_status",
        "Read simulated vehicle state, coolant temperature, battery voltage and DTC counts through UDS DID F100 on OK8MP M7 FlexCAN1.",
        tool_ok8mp_can_status_execute);

    REGISTER_TOOL(
        "can_set_ecu_state",
        "Set the ESP32 simulated ECU state through UDS service 0x2E.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("state", "Target ECU state",
                            "\"normal\",\"overtemp\",\"low_voltage\"")
            TOOL_SCHEMA_END_REQUIRED("\"state\""),
        tool_ok8mp_can_set_state_execute);

    REGISTER_TOOL(
        "can_dtc",
        "Read or clear diagnostic trouble codes through UDS services 0x19 and 0x14.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("action", "DTC operation", "\"read\",\"clear\"")
            TOOL_SCHEMA_END_REQUIRED("\"action\""),
        tool_ok8mp_can_dtc_execute);

    REGISTER_TOOL(
        "can_obd_query",
        "Run an SAE J1979 OBD-II query over ISO-TP and return structured vehicle data.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("query", "OBD-II query",
                            "\"supported_pids\",\"coolant\",\"battery_voltage\",\"dtc\",\"clear_dtc\",\"vin\"")
            TOOL_SCHEMA_END_REQUIRED("\"query\""),
        tool_ok8mp_can_obd_execute);

    REGISTER_TOOL(
        "can_uds_query",
        "Run a UDS VIN read or TesterPresent request over the OK8MP CAN diagnostic channel.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("operation", "UDS operation",
                            "\"read_vin\",\"tester_present\"")
            TOOL_SCHEMA_END_REQUIRED("\"operation\""),
        tool_ok8mp_can_uds_execute);
#endif
    ...
    /* Music tools require the openvela multimedia framework. */
#ifdef CONFIG_MEDIA
    /* Music search */
