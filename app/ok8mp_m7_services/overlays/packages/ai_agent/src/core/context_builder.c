...
int context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- CAN DIAGNOSTIC IS LIVE-ONLY: for ECU online diagnosis, CAN health "
        "probe, 0x700/0x708 probe, or an explicit can_diagnostic request, "
        "you MUST call can_diagnostic in this turn before reporting any "
        "diagnostic result. Never reuse, infer, or reproduce a prior probe "
        "result. If the tool is unavailable or fails, report that failure only.\n"
