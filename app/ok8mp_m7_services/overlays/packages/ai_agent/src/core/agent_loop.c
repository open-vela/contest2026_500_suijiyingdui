    ...
    const struct timespec* t1)
{
    ...
    /* CLOCK_MONOTONIC must not jump when NTP/TLS corrects CLOCK_REALTIME,
     * but still fail safely if a platform implementation misbehaves. */
    if (sec_diff < 0) {
        syslog(LOG_WARNING, "[%s] Monotonic clock went backwards\n", TAG);
        return 0;
    }
    ...
    elapsed_ms = (uint64_t)sec_diff * 1000 + (uint64_t)nsec_diff / 1000000;
...
static const char* kw_can_diagnostic[] = {
    "can_diagnostic", "CAN诊断", "CAN 诊断", "在线诊断",
    "诊断 ECU", "ECU 在线", "ECU是否在线", "ECU 是否在线", NULL };
...
{
    /* An ECU online verdict is evidence, not conversational knowledge.
     * Route explicit diagnostic requests directly to the live CAN probe so
     * an LLM cannot answer from a previous turn without transmitting 0x700.
     */
    if (contains_any(text, kw_can_diagnostic)) {
        char* reply = calloc(1, 512);
        if (reply) {
            int ret = tool_registry_execute("can_diagnostic", "{}", reply,
                                            512);
            if (ret < 0 && reply[0] == '\0') {
                snprintf(reply, 512,
                         "{\"ok\":false,\"state\":\"tool_error\","
                         "\"tool\":\"can_diagnostic\",\"error\":%d}",
                         ret);
            }
            s_fast_path_count++;
            syslog(LOG_INFO,
                   "[%s] Forced live CAN diagnostic (fast=%d llm=%d)\n",
    ...
    cJSON* messages)
{
    ...
    if (err == OK && resp.text && resp.text_len > 0) {
        if (timed_out) {
            syslog(LOG_WARNING,
                "[%s] Force-finish LLM completed after %" PRIu32
                " ms; accepting valid response\n",
                TAG, ms);
        }
    ...
    bool* out_timed_out)
{
    ...
    if (err == OK && final_resp.text && final_resp.text_len > 0) {
        if (timed_out) {
            syslog(LOG_WARNING,
                "[%s] Task-complete LLM finished after %" PRIu32
                " ms; accepting valid response\n",
                TAG, ms);
        }
    ...
    const agent_msg_t* msg)
{
    ...
    for (iteration = 0; iteration < AGENT_AI_AGENT_MAX_TOOL_ITER;
        ...
        /* This is a post-call latency monitor, not an asynchronous
         * watchdog.  When llm_chat_tools returned OK, its JSON and tool
         * calls were already validated; never discard that valid result
         * merely because the synchronous call exceeded the target. */
        if (llm_call_timed_out(latency_ms)) {
            syslog(LOG_WARNING,
                "[%s] LLM latency: call completed after %" PRIu32
                " ms (target %ds); accepting valid response\n",
                TAG, latency_ms, AGENT_LLM_TIMEOUT_SEC);
        }
        ...
        if (!resp.tool_use) {
            ...
            if (cascade_retry) {
                ...
                if (prem_idx >= 0 && prem_idx != router_idx) {
                    ...
                    /* Only report a timeout when the call actually failed.
                     * A validated late response remains usable. */
...
send_reply:
    ...
    if (final_text && msg->content
        && complexity == LLM_COMPLEXITY_SIMPLE
        && !watchdog_fired
        && strcmp(final_text, LLM_TIMEOUT_MSG) != 0
        && strcmp(final_text, LLM_TIMEOUT_TASK_COMPLETE_MSG) != 0) {
