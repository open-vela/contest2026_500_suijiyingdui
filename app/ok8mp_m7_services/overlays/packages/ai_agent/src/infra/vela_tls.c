...
#ifdef CONFIG_AI_AGENT_TLS_VERIFY_REQUIRED
#include "infra/agent_ca_bundle.h"
#endif
...
static size_t decode_chunked(char* buf, size_t len)
{
    ...
    while (src < end) {
        ...
        /* Validate the hex size.  RFC 7230 permits an optional chunk
         * extension beginning with ';' before the CRLF. */
...
/* Return true once a complete chunked HTTP body, including the terminating
 * zero-size chunk and optional trailers, has arrived.  A keep-alive server
 * does not close the socket after the response, so waiting for EOF here
 * would otherwise consume the full SO_RCVTIMEO even though the response is
 * already complete.
 */
static bool chunked_body_complete(const char* buf, size_t len)
{
    const char* src = buf;
    const char* end = buf + len;

    while (src < end) {
        const char* crlf = memmem(src, (size_t)(end - src), "\r\n", 2);
        char* endptr;
        ...
        chunk_sz = strtoul(src, &endptr, 16);
        if (endptr == src) {
            return false;
        }

        /* RFC 7230 permits optional chunk extensions after the hex size. */
        ...
        src = crlf + 2;
        if (chunk_sz == 0) {
            /* No trailers: 0\r\n\r\n.  With trailers, the trailer block is
             * itself terminated by an empty line. */
            if ((size_t)(end - src) >= 2 && src[0] == '\r' && src[1] == '\n') {
                return true;
            }
        ...
        if (chunk_sz > (unsigned long)(end - src)) {
            return false;
        }
        src += chunk_sz;
...
static int tls_ctx_connect(tls_ctx_t* ctx, const char* host, const char* port)
{
...
#ifdef CONFIG_AI_AGENT_TLS_VERIFY_REQUIRED
        syslog(LOG_ERR,
            "[%s] Clock invalid for strict X.509 verification: %ld\n",
            TAG, (long)now);
        return VELA_TLS_ERR_HANDSHAKE;
#else
    ...
    ret = mbedtls_x509_crt_parse(&ctx->ca,
        (const unsigned char *)g_agent_ca_bundle,
        sizeof(g_agent_ca_bundle));
    if (ret != 0) {
        syslog(LOG_ERR, "[%s] CA bundle parse ret=-0x%04x\n", TAG, -ret);
        return VELA_TLS_ERR_HANDSHAKE;
    }
#endif
...
#ifdef CONFIG_AI_AGENT_TLS_VERIFY_REQUIRED
    {
        uint32_t verify_flags = mbedtls_ssl_get_verify_result(&ctx->ssl);

        if (verify_flags != 0) {
            syslog(LOG_ERR, "[%s] X.509 verification flags=0x%08lx\n",
                TAG, (unsigned long)verify_flags);
            return VELA_TLS_ERR_HANDSHAKE;
        }
        syslog(LOG_INFO, "[%s] X.509 certificate and hostname verified\n",
            TAG);
    }
#endif
...
#define HDR_APPEND(fmt, ...)                                    \
    ...
    HDR_APPEND("Host: %s\r\n", host);
    /* The Cortex-M7 bring-up prioritizes deterministic completion over TLS
     * connection reuse.  Some LLM endpoints keep an HTTP/1.1 connection open
     * even when their body framing is absent or non-standard, which made the
     * reader wait for SO_RCVTIMEO after the JSON was already received. */
    ...
    size_t* out_body_len, bool* out_keep_alive)
{
    ...
    /* Requests currently carry Connection: close, so never return this
     * context to the keep-alive pool even if the peer omits or contradicts
    ...
    syslog(LOG_INFO,
        "[%s] HTTP response framing: status=%d length=%ld chunked=%d initial=%zu\n",
        TAG, http_status, content_length, chunked, initial);

    /* Once body bytes have started arriving, a non-streaming LLM response
     * should not have a long inter-packet pause.  Use a short idle timeout
     * for the final read so an absent/non-standard length terminator cannot
     * consume the full 120-second first-byte timeout. */
    bool body_idle_timeout_set = false;
    if (resp_pos > 0 && ctx->net.fd >= 0) {
        struct timeval tv_idle = { .tv_sec = 2, .tv_usec = 0 };
        setsockopt(ctx->net.fd, SOL_SOCKET, SO_RCVTIMEO,
            &tv_idle, sizeof(tv_idle));
        body_idle_timeout_set = true;
    }

    /* Keep reading body.  Content-Length responses stop at the declared
     * byte count.  Chunked keep-alive responses must stop at the terminal
     * zero-size chunk instead of waiting for the peer to close the socket. */
    if (!eof) {
        while (resp_pos < resp_cap - 1) {
            ...
            if (ret > 0) {
                resp_pos += (size_t)ret;
                if (!body_idle_timeout_set && ctx->net.fd >= 0) {
                    struct timeval tv_idle = { .tv_sec = 2, .tv_usec = 0 };
                    setsockopt(ctx->net.fd, SOL_SOCKET, SO_RCVTIMEO,
                        &tv_idle, sizeof(tv_idle));
                    body_idle_timeout_set = true;
