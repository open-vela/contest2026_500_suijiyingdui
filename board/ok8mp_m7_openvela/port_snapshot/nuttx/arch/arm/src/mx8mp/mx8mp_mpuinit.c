...
void mx8mp_mpu_initialize(void)
{
  ...
  /* Region 4: reserve the first 32 KiB of M7 OCRAM for FEC DMA descriptors
   * and packet buffers.  This region overrides the broad device mapping and
   * is normal, non-cacheable memory, so no D-cache maintenance is required.
   */

  mpu_configure_region(MX8M_M7_OCRAM, 32 * 1024,
                      MPU_RASR_TEX_NOR  |  /* Normal
                                            * Not Cacheable */
                      MPU_RASR_B        |  /* Bufferable
                                            * Not Shareable */
  ...
  /* Region 5 DDR[0x4000_0000 - 0x7FFF_FFFF]
   * Normal type, not shareable, cacheable
   */
